\section{Thiết kế Dashboard Tableau}

Dashboard tổng được thiết kế để trả lời bài toán chung trước, sau đó cho phép người xem đi sâu vào từng hướng phân tích. Thứ tự đọc đề xuất bắt đầu từ bức tranh tổng quan về phát triển bền vững, sau đó chuyển sang từng nhóm chỉ số cụ thể. Các bộ lọc chung gồm năm, khu vực, nhóm thu nhập và quốc gia, nhờ đó người xem có thể giữ cùng một phạm vi phân tích khi so sánh kinh tế, giáo dục, sức khỏe và môi trường.

Trang tổng quan đặt cạnh các chỉ số đại diện cho bốn khía cạnh để hỗ trợ so sánh nhanh giữa các quốc gia hoặc khu vực. Từ trang này, người xem có thể chuyển sang khu vực kinh tế với các biểu đồ GDP bình quân đầu người, tăng trưởng GDP, quan hệ GDP--nghèo và bản đồ GDP bình quân đầu người. Các khu vực còn lại lần lượt trình bày chỉ số nhập học, biết chữ và chi tiêu giáo dục; tuổi thọ, tử vong trẻ em và chi tiêu y tế; phát thải CO\textsubscript{2}, năng lượng tái tạo và diện tích rừng. Cách tổ chức này giữ mạch đọc từ tổng quan đến chi tiết, đồng thời làm rõ vai trò của từng nhóm chỉ số trong bài toán chung.

{\captionsetup{justification=centering,singlelinecheck=true}

\reportfigure{images/tableau/dashboard_overview.png}{Tab tổng quan của dashboard.}{fig:tableau-overview-dashboard}

\reportfigure{images/tableau/dashboard_economy.png}{Tab phân tích kinh tế.}{fig:tableau-economy-dashboard}

\reportfigure{images/tableau/dashboard_education.png}{Tab phân tích giáo dục.}{fig:tableau-education-dashboard}

\reportfigure{images/tableau/dashboard_health.png}{Tab phân tích sức khỏe.}{fig:tableau-health-dashboard}

\reportfigure{images/tableau/dashboard_environment.png}{Tab phân tích môi trường.}{fig:tableau-environment-dashboard}

}
